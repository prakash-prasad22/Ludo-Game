...
// Play continuous background music (e.g. 'home' theme)
export const playBackgroundSound = (soundName) => {
  try {
    // Stop any currently playing background track first
    stopBackgroundSound();

    const soundPath = getSoundPath(soundName);
    backgroundPlayer = createAudioPlayer(soundPath);
    backgroundPlayer.loop = true;
    backgroundPlayer.play();
  } catch (e) {
    console.log(`Cannot play background sound: ${soundName}`, e);
  }
};
...
export const stopBackgroundSound = () => {
  try {
    ...
    console.log(`Cannot stop background sound`, e);
  }
};

// For quick one-shot SFX (dice rolls, clicks, etc.)
export const playSound = (soundName) => {
  try {
    const soundPath = getSoundPath(soundName);
    const player = createAudioPlayer(soundPath);
    player.play();
  } catch (e) {
    console.log(`Cannot play sound effect: ${soundName}`, e);
  }
};
